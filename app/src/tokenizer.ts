...
export function selectMessagesToSendSafely(messages: OpenAIMessage[]) {
    ...
    const insertedSystemMessage = serializeChatMLMessage('system', 'Varios mensajes no incluidos debido a limitaciones de espacio');
    ...
    if (removed) {
        output.splice(1, 0, {
            role: 'system',
            content: 'Varios mensajes no incluidos debido a limitaciones de espacio',
        });
    }
